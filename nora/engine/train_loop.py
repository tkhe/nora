...
class Hook(object):
    """
    Base class for hooks that can be registered with :class:`Trainer`.
    ...
    Notes:
        ...
        1. In the hook method, users can access ``self.trainer`` to access more
           properties about the context (e.g., model, current iteration).
    ...
    def priority(self) -> Priority:
        return self._priority if hasattr(self, "_priority") else Priority.NORMAL

    @priority.setter
    def priority(self, val):
        self._priority = Priority.get_priority(val)
...
class Trainer(object):
    ...
    def register_hook(self, hook: Optional[Hook]):
        """
        Register single hook to the trainer.

        Args:
            hook (Optional[Hook]): hook to be registered.
        """
        if hook is not None:
            ...
            for i in range(len(self._hooks) - 1, -1, -1):
                if Priority.get_priority(hook.priority) >= Priority.get_priority(self._hooks[i].priority):
    ...
    def register_hooks(self, hooks: List[Optional[Hook]]):
        """
...
class SimpleTrainer(Trainer):
    ...
    def write_metrics(
        ...
        """
        metrics_dict = {k: (v.detach().cpu().item() if isinstance(v, torch.Tensor) else v) for k, v in loss_dict.items()}
        ...
        if comm.is_main_process():
            ...
            storage.put_scalar(f"{prefix}total_loss", total_losses_reduced, cur_iter=cur_iter)
...
class AMPTrainer(SimpleTrainer):
    ...
    def run_step(self):
        ...
        """
        assert self.model.training, f"[{self.__class__.__name__}] model was changed to eval mode!"
        assert torch.cuda.is_available(), f"[{self.__class__.__name__}] CUDA is required for AMP training!"
