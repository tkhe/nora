...
from nora.utils.file_io import PathManagerBase
from .loading import align_and_update_state_dicts
...
class Checkpointer:
    ...
    def __init__(
        ...
        *,
        ...
        path_manager: Optional[PathManagerBase] = None,
        **checkpointables: Any,
    ):
        """
        Args:
    ...
    def add_checkpointable(self, key: str, checkpointable: Any):
        """
    ...
    def load(self, path: str, checkpointables: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        for key in self.checkpointables if checkpointables is None else checkpointables:
            if key in checkpoint:
                self.logger.info(f"Loading {key} from {path} ...")
    ...
    def _load_file(self, f: str) -> Dict[str, Any]:
        ...
        with self.path_manager.open(f, "rb") as file:
            # TODO: The cast is redundant.
            loaded = torch.load(cast(IO[bytes], file), map_location=torch.device("cpu"))
        ...
        if "__author__" not in loaded and "__author__" in queries:
            author = queries.pop("__author__")
            assert len(author) == 1

            author = author[0]
            loaded["__author__"] = author

        if "__author__" in loaded:
            author = loaded["__author__"]
            self.logger.info(f"Reading a file from '{author}'")
            ...
            if k in model_state_dict:
                ...
                if shape_model != shape_checkpoint:
                    ...
                    if has_observer_base_classes:
                        ...
                        cls_to_skip = (ObserverBase, FakeQuantizeBase)
    ...
    def _log_incompatible_keys(self, incompatible: _IncompatibleKeys):
        """
        ...
        for k, shape_checkpoint, shape_model in incompatible.incorrect_shapes:
            self.logger.warning(f"Skip loading parameter '{k}' to the model due to incompatible shapes: {shape_checkpoint} in the checkpoint but {shape_model} in the model! You might want to double check if this is expected.")
    ...
    def _convert_ndarray_to_tensor(self, state_dict: Dict[str, Any]):
        """
...
class PeriodicCheckpointer:
    ...
    def step(self, iteration: int, **kwargs: Any):
        ...
        if (iteration + 1) % self.period == 0:
            self.checkpointer.save(f"{self.file_prefix}_{iteration + 1:07d}", **additional_state)

            if self.max_to_keep is not None:
                self.recent_checkpoints.append(self.checkpointer.get_checkpoint_file())
                if len(self.recent_checkpoints) > self.max_to_keep:
                    file_to_delete = self.recent_checkpoints.pop(0)
                    if self.path_manager.exists(file_to_delete) and not file_to_delete.endswith(f"{self.file_prefix}_final.pth"):
...
def _strip_prefix_if_present(state_dict: Dict[str, Any], prefix: str):
    """
