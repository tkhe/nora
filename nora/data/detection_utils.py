...
def check_image_size(dataset_dict, image):
    ...
    if "width" in dataset_dict or "height" in dataset_dict:
        ...
        if not image_wh == expected_wh:
            raise SizeMismatchError(f"Mismatched image shape{' for image' + dataset_dict['file_name'] if 'file_name' in dataset_dict else ''}, got {image_wh}, expect {expected_wh}. Please check the width/height in your annotation.")
...
def transform_instance_annotations(annotation, transforms, image_size, *, keypoint_hflip_indices=None):
    ...
    bbox = transforms.apply_boxes(np.array([bbox]))[0].clip(min=0)
    ...
    if "segmentation" in annotation:
        ...
        if isinstance(segm, list):
            ...
            raise ValueError(f"Cannot transform segmentation of type '{type(segm)}'! Supported types are: polygons as list[list[float] or ndarray], COCO-style RLE as a dict.")
...
def transform_keypoint_annotations(keypoints, transforms, image_size, keypoint_hflip_indices=None):
    ...
    # do_hflip = np.sign(probe[1][0] - probe[0][0]) != np.sign(probe_aug[1][0] - probe_aug[0][0])
    ...
    if do_hflip:
        ...
        if len(keypoints) != len(keypoint_hflip_indices):
            raise ValueError(f"Keypoint data has {len(keypoints)} points, but metadata contains {len(keypoint_hflip_indices)} points!")
        ...
        if mask_format == "polygon":
            ...
            for segm in segms:
                if isinstance(segm, list):
                    ...
                    raise ValueError(f"Cannot convert segmentation of type '{type(segm)}' to BitMasks! Supported types are: polygons as list[list[float] or ndarray], COCO-style RLE as a dict, or a binary segmentation mask  in a 2D numpy array of shape HxW.")
