...
class TrainingSampler(Sampler):
    ...
    This note is true for all samplers in nora.
    """
...
class RepeatFactorTrainingSampler(Sampler):
    ...
    def repeat_factors_from_category_frequency(dataset_dicts, repeat_thresh, sqrt=True):
        ...
        category_rep = {
            cat_id: max(1.0, (math.sqrt(repeat_thresh / cat_freq) if sqrt else (repeat_thresh / cat_freq)))
            for cat_id, cat_freq in category_freq.items()
        }
        for cat_id in sorted(category_rep.keys()):
            logger.info(f"Cat ID {cat_id}: freq={category_freq[cat_id]:.2f}, rep={category_rep[cat_id]:.2f}")
