...
class Transform(ABC):
    ...
    def apply_image(self, image: np.ndarray) -> np.ndarray:
        ...
        Args:
            image (ndarray): of shape NxHxWxC, or HxWxC or HxW. The array can be
    ...
    def apply_boxes(self, boxes: np.ndarray) -> np.ndarray:
        ...
        the corner points and use their minimum/maximum to create new
        axis-aligned boxes. Note that this default may change sizes of your
        boxes, e.g. after rotations.

        Args:
            boxes (ndarray): Nx4 floating point array of XYXY format in absolute
                coordinates.
        Returns:
            ndarray: boxes after apply the transformation.
    ...
    def register_type(cls, data_type: str, func: Optional[Callable] = None):
        ...
        Examples:
            ...
            >>> HorizontalFlipTransform.register_type("voxel", func)

            >>> # or, use it as a decorator
            >>> @HorizontalFlipTransform.register_type("voxel")
            >>> def func(flip_transform, voxel_data):
            >>>     return transformed_voxel_data
        ...
        assert len(argspec.args) == 2, f"You can only register a function that takes two positional arguments to a Transform! Got a function with spec {str(argspec)}"
    ...
    def __repr__(self) -> str:
        ...
        try:
            ...
            for name, param in sig.parameters.items():
                ...
                assert hasattr(self, name), f"Attribute {name} not found! Default __repr__ only works if attributes match the constructor."
...
class TransformList(Transform):
    ...
    def __init__(self, transforms: List[Transform]):
        ...
        tfms_flatten = []
        for t in transforms:
            assert isinstance(t, Transform), f"TransformList requires a list of Transform. Got type {type(t)}!"
    ...
    def __repr__(self) -> str:
        ...
        return f"TransformList[{', '.join(msgs)}]"
...
class PadTransform(Transform):
    ...
    def apply_image(self, image: np.ndarray) -> np.ndarray:
        ...
        return np.pad(image, padding, mode="constant", constant_values=self.pad_value)
    ...
    def apply_segmentation(self, segmentation: np.ndarray) -> np.ndarray:
        ...
        return np.pad(segmentation, padding, mode="constant", constant_values=self.seg_pad_value)
    ...
    def inverse(self) -> Transform:
        assert (self.orig_w is not None and self.orig_h is not None), "orig_w, orig_h are required for PadTransform to be invertible!"
...
class CropTransform(Transform):
    def __init__(
        ...
        """
        Args:
            x0, y0, w, h (int): crop the image(s) by image[y0:y0+h, x0:x0+w].
    ...
    def apply_image(self, image: np.ndarray) -> np.ndarray:
        ...
        if len(image.shape) <= 3:
            return image[self.y0 : self.y0 + self.h, self.x0 : self.x0 + self.w]
        else:
            return image[..., self.y0 : self.y0 + self.h, self.x0 : self.x0 + self.w, :]
    ...
    def inverse(self) -> Transform:
        assert (self.orig_w is not None and self.orig_h is not None), "orig_w, orig_h are required for CropTransform to be invertible!"
...
class ResizeTransform(Transform):
    ...
    def __init__(
        ...
        Args:
            ...
            interp: interpolation methods, defaults to bilinear.
        """
        if interp is None:
            interp = "bilinear"
    ...
    def apply_image(self, image: np.ndarray, interp: Optional[str] = None) -> np.ndarray:
        ...
        interp_method = INTERPOLATION_METHOD[interp_method]

        ret = cv2.resize(image, (self.new_w, self.new_h), interpolation=interp_method)
...
class RotationTransform(Transform):
    ...
    def apply_image(self, image: np.ndarray, interp: Optional[str] = None) -> np.ndarray:
        """
        image should be a numpy array, formatted as Height * Width * Nchannels
        """
        ...
        return cv2.warpAffine(image, self.rm_image, (self.bound_w, self.bound_h), flags=interp)
    ...
    def inverse(self):
        ...
        """
        # Not possible to inverse if a part of the image is lost
        if not self.expand:
            raise NotImplementedError()
...
@HorizontalFlipTransform.register_type("rotated_boxes")
def HorizontalFlip_rotated_box(transform, rotated_boxes):
    """
...
@ResizeTransform.register_type("rotated_boxes")
