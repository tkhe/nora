...
def _check_image_dtype(image: np.ndarray):
    assert isinstance(image, np.ndarray), f"[Augmentation] Needs an numpy array, but got a {type(image)}"
    assert not isinstance(image.dtype, np.integer) or (image.dtype == np.uint8), f"[Augmentation] Got image of type {image.dtype}, use uint8 or floating points instead!"
    assert image.ndim in [2, 3], image.ndim
...
def _get_aug_input_args(aug, aug_input) -> List[Any]:
        ...
        if len(prms) == 1:
            ...
            for name, prm in prms:
                if prm.kind in (inspect.Parameter.VAR_POSITIONAL, inspect.Parameter.VAR_KEYWORD):
                    raise TypeError(f"The default implementation of `{type(aug)}.__call__` does not allow `{type(aug)}.get_transform` to use variable-length arguments (*args, **kwargs)! If arguments are unknown, reimplement `__call__` instead.")
                names.append(name)
    ...
    for f in aug.input_args:
        try:
            ...
            raise AttributeError(f"{type(aug)}.get_transform needs input attribute '{f}', but it is not an attribute of {type(aug_input)}!") from e
...
class AugInput:
    ...
    * The input must have a ``transform(tfm: Transform)`` method which
