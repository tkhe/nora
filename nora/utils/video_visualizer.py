...
class VideoVisualizer:
    def __init__(self, metadata, instance_mode=ColorMode.IMAGE):
        ...
        """
        assert instance_mode in [ColorMode.IMAGE, ColorMode.IMAGE_BW], "Other mode not supported yet."
    ...
    def draw_instance_predictions(self, frame, predictions):
        ...
        if self._instance_mode == ColorMode.IMAGE_BW:
            ...
            frame_visualizer.output.reset_image(frame_visualizer._create_grayscale_image((masks.any(dim=0) > 0).numpy() if masks is not None else None))
            alpha = 0.3
        else:
            alpha = 0.5

        labels = None if labels is None else [y[0] for y in filter(lambda x: x[1], zip(labels, visibilities))]
        assigned_colors = None if colors is None else [y[0] for y in filter(lambda x: x[1], zip(colors, visibilities))]
    ...
    def _assign_colors_by_id(self, instances: Instances) -> List:
        ...
        for id in instances.ID:
            if id in self._assigned_colors:
                ...
                assert len(self._color_idx_set) >= 1, f"Number of id exceeded maximum, max = {self._max_num_instances}"
