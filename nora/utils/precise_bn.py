...
class _MeanOfBatchVarianceEstimator:
    ...
    def __init__(self, mean_buffer: torch.Tensor, var_buffer: torch.Tensor):
        self.pop_mean: torch.Tensor = torch.zeros_like(mean_buffer)
        self.pop_var: torch.Tensor = torch.zeros_like(var_buffer)
        self.ind = 0

    def update(self, batch_mean: torch.Tensor, batch_var: torch.Tensor, batch_size: int):
        self.ind += 1
...
def update_bn_stats(
    ...
    def get_bn_batch_size_hook(module: nn.Module, input: Tuple[torch.Tensor]) -> Tuple[torch.Tensor]:
    ...
    assert ind == num_iters - 1, f"update_bn_stats is meant to run for {num_iters} iterations, but the dataloader stops at {ind} iterations."
...
def get_bn_modules(model: nn.Module) -> List[nn.Module]:
    ...
    BN_MODULE_TYPES for a list of all modules that are
