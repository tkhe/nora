...
class JSONWriter(EventWriter):
    ...
    def __init__(self, json_file, window_size: int = 20):
        """
        Args:
    ...
    def write(self):
        ...
        for itr, scalars_per_iter in to_save.items():
            scalars_per_iter["iteration"] = itr + 1
...
class TensorboardXWriter(EventWriter):
    ...
    def write(self):
        ...
        if len(storage._vis_data) >= 1:
            for image_name, image, step_num in storage._vis_data:
                self._writer.add_image(image_name, image, step_num)
...
class CommonMetricPrinter(EventWriter):
    ...
    def __init__(self, max_iter: Optional[int] = None, window_size: int = 20):
        ...
        """
        self.logger = logging.getLogger("nora.utils.events")
    ...
    def write(self):
        ...
        try:
            lr = f"{storage.history('lr').latest():.5g}"
        except KeyError:
            lr = "N/A"
        ...
        self.logger.info(
            str.format(
                " {eta}iter: {iter}  {losses}  {non_losses}  {avg_time}{last_time}{avg_data_time}{last_data_time} lr: {lr}  {memory}",
                eta=f"eta: {eta_string}  " if eta_string else "",
                iter=f"[{iteration + 1} / {self._max_iter}]",
                losses="  ".join(
                    [
                        f"{k}: {v.median(storage.count_samples(k, self._window_size)):.4g}"
                ...
                avg_time=f"time: {avg_iter_time:.4f}  " if avg_iter_time is not None else "",
                last_time=f"last_time: {last_iter_time:.4f}  " if last_iter_time is not None else "",
                avg_data_time=f"data_time: {avg_data_time:.4f}  " if avg_data_time is not None else "",
                last_data_time=f"last_data_time: {last_data_time:.4f}  " if last_data_time is not None else "",
                lr=lr,
...
class EventStorage:
    ...
    def __init__(self, start_iter: int = 0):
        """
        Args:
    ...
    def put_image(self, image_name, image_tensor):
        """
        Add an `image_tensor` associated with `image_name`, to be shown on
        tensorboard.

        Args:
            image_name (str): The name of the image to put into tensorboard.
            image_tensor (torch.Tensor or numpy.array): An `uint8` or `float`
                Tensor of shape `[channel, height, width]` where `channel` is
                3. The image format should be RGB. The elements in image_tensor
                can either have values in [0, 1] (float32) or [0, 255] (uint8).
                The `image_tensor` will be visualized in tensorboard.
        """
        self._vis_data.append((image_name, image_tensor, self._iter))
    ...
    def latest_with_smoothing_hint(self, window_size=20):
        ...
        for k, (v, itr) in self._latest_scalars.items():
            result[k] = (self._history[k].median(self.count_samples(k, window_size)) if self._smoothing_hints[k] else v, itr)
        return result
