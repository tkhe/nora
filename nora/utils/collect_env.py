...
def collect_env_info():
    ...
    try:
        ...
        data.append(("nora", nora.__version__ + " @" + os.path.dirname(nora.__file__)))
    except ImportError:
        data.append(("nora", "failed to import"))
    except AttributeError:
        data.append(("nora", "imported a wrong installation"))

    try:
        import nora._C as _C
    except ImportError as e:
        data.append(("nora._C", f"not built correctly: {e}"))
        ...
        if has_cuda and sys.platform != "win32":
            try:
                so_file = importlib.util.find_spec("nora._C").origin
            except (ImportError, AttributeError):
                pass
            else:
                data.append(("nora arch flags", detect_compute_compatibility(CUDA_HOME, so_file)))
    else:
        ...
        if has_cuda and getattr(_C, "has_cuda", lambda: True)():
            data.append(("nora arch flags", detect_compute_compatibility(CUDA_HOME, _C.__file__)))
