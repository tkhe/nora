...
from nora.utils.comm import all_gather
from nora.utils.comm import is_main_process
from nora.utils.comm import synchronize
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            if self._compute_boundary_iou:
                ...
                self._b_conf_matrix += np.bincount((self._num_classes + 1) * b_pred.reshape(-1) + b_gt.reshape(-1), minlength=self._conf_matrix.size).reshape(self._conf_matrix.shape)
    ...
    def encode_json_sem_seg(self, sem_seg, input_file_name):
        ...
        for label in np.unique(sem_seg):
            if self._contiguous_id_to_dataset_id is not None:
                assert label in self._contiguous_id_to_dataset_id, f"Label {label} is not in the metadata info for {self._dataset_name}"
