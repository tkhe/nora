...
from nora.utils.logger import log_every_n_seconds
        ...
        for evaluator in self._evaluators:
            ...
            if is_main_process() and result is not None:
                for k, v in result.items():
                    assert k not in results, f"Different evaluators produce results with the same key {k}"
...
def inference_on_dataset(
    ...
    logger.info(f"Total inference time: {total_time_str} ({total_time / (total - num_warmup):.6f} s / iter per device, on {num_devices} devices)")
    total_compute_time_str = str(datetime.timedelta(seconds=int(total_compute_time)))
    logger.info(f"Total inference pure compute time: {total_compute_time_str} ({total_compute_time / (total - num_warmup):.6f} s / iter per device, on {num_devices} devices)")
