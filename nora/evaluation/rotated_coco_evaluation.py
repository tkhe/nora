...
from nora.structures import pairwise_iou_rotated
from nora.utils.file_io import PathManager
from .coco_evaluation import COCOEvaluator

__all__ = ["RotatedCOCOEvaluator"]
    ...
    def boxlist_to_tensor(boxlist, output_box_dim):
        ...
        if input_box_dim != output_box_dim:
            if input_box_dim == 4 and output_box_dim == 5:
                ...
                raise Exception(f"Unable to convert from {input_box_dim}-dim box to {output_box_dim}-dim box")
        return box_tensor
...
class RotatedCOCOEvaluator(COCOEvaluator):
    ...
    def process(self, inputs, outputs):
        """
        Args:
            inputs: the inputs to a detection model.
                It is a list of dict. Each dict corresponds to an image and
                contains keys like "height", "width", "file_name", "image_id".
            outputs: the outputs of a detection model. It is a list of dicts with key
    ...
    def _eval_predictions(self, predictions, img_ids=None):  # img_ids: unused
        ...
        assert self._tasks is None or set(self._tasks) == {"bbox"}, "[RotatedCOCOEvaluator] Only bbox evaluation is supported"

        # cocoapi does not handle empty results very well
        coco_eval = self._evaluate_predictions_on_coco(self._coco_api, coco_results) if len(coco_results) > 0 else None
