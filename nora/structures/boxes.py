...
class BoxMode(IntEnum):
    ...
    def convert(box: _RawBoxType, from_mode: "BoxMode", to_mode: "BoxMode") -> _RawBoxType:
        ...
        assert (
            to_mode not in [BoxMode.XYXY_REL, BoxMode.XYWH_REL]
        ...
        if from_mode == BoxMode.XYWHA_ABS and to_mode == BoxMode.XYXY_ABS:
            ...
            if to_mode == BoxMode.XYXY_ABS and from_mode == BoxMode.XYWH_ABS:
                ...
                arr[:, 2] -= arr[:, 0]
                arr[:, 3] -= arr[:, 1]
            elif from_mode == BoxMode.CXCYWH_ABS and to_mode == BoxMode.XYXY_ABS:
                arr = torch.cat((arr[:, 0:2] - arr[:, 2:4] / 2, arr[:, 0:2] + arr[:, 2:4] / 2), dim=1)
            elif from_mode == BoxMode.XYXY_ABS and to_mode == BoxMode.CXCYWH_ABS:
                arr = torch.cat(((arr[:, 2:4] + arr[:, 0:2]) / 2, arr[:, 2:4] - arr[:, 0:2]), dim=1)
            else:
...
def matched_pairwise_iou(boxes1: Boxes, boxes2: Boxes) -> torch.Tensor:
    ...
    Returns:
        Tensor: iou, sized [N].
    """
    assert len(boxes1) == len(boxes2), f"boxlists should have the same number of entries, got {len(boxes1)}, {len(boxes2)}"
