...
from nora.layers import ROIAlign
from nora.utils.memory import retry_if_cuda_oom
...
class BitMasks:
    ...
    def __getitem__(self, item: Union[int, slice, torch.BoolTensor]) -> "BitMasks":
        ...
        m = self.tensor[item]
        assert m.dim() == 3, f"Indexing on BitMasks with {item} returns a tensor with shape {m.shape}!"
...
class PolygonMasks:
    ...
    def __init__(self, polygons: List[List[Union[torch.Tensor, np.ndarray]]]):
        ...
        if not isinstance(polygons, list):
            raise ValueError(f"Cannot create PolygonMasks: Expect a list of list of polygons per image. Got '{type(polygons)}' instead.")
        ...
        def process_polygons(polygons_per_instance: List[Union[torch.Tensor, np.ndarray]]) -> List[np.ndarray]:
            if not isinstance(polygons_per_instance, list):
                raise ValueError(f"Cannot create polygons: Expect a list of polygons per instance. Got '{type(polygons_per_instance)}' instead.")
...
class ROIMasks:
    ...
    def to_bitmasks(self, boxes: torch.Tensor, height, width, threshold=0.5):
        ...
        """
        from nora.layers.mask_ops import paste_masks_in_image
        from nora.layers.mask_ops import _paste_masks_tensor_shape
