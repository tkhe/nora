...
class ExponentialParamScheduler(ParamScheduler):
    ...
    def __init__(self, start_value: float, decay: float):
        self._start_value = start_value
        self._decay = decay
    ...
    def __init__(
        ...
        * milestones=[3, 6, 9], values=[1, 0.1, 0.01]  (ParamScheduler is scale-invariant)
        ...
        if milestones is None:
            ...
            if not (isinstance(milestones, Sequence) and len(milestones) == len(values) - int(num_updates is not None)):
                raise ValueError(f"MultiStep scheduler requires a list of {len(values) - int(num_updates is not None)} miletones")
        ...
        for milestone in self._milestones:
            ...
            if milestone >= self._num_updates:
                raise ValueError(f"Milestone must be smaller than total number of updates: num_updates={self._num_updates}, milestone={milestone}")

            # Must be in ascending order
            if start_epoch >= milestone:
                raise ValueError(f"Milestone must be smaller than start epoch: start_epoch={start_epoch}, milestone={milestone}")

            start_epoch = milestone

    def __call__(self, where: float) -> float:
        if where > 1.0:
            raise RuntimeError(f"where in MultiStepParamScheduler must be in [0, 1]: got {where}")
...
class PolynomialDecayParamScheduler(ParamScheduler):
    ...
    def __init__(self, base_value: float, power: float):
...
class StepParamScheduler(ParamScheduler):
    ...
    def __init__(self, num_updates: Union[int, float], values: List[float]):
        if num_updates <= 0:
...
class StepWithFixedGammaParamScheduler(ParamScheduler):
    ...
    def __init__(self, base_value: float, num_decays: int, gamma: float, num_updates: int):
