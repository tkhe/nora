...
def create_instances(predictions, image_size):
    ...
    score = score[chosen]
    box = np.asarray([predictions[i]["bbox"] for i in chosen]).reshape(-1, 4)
    box = BoxMode.convert(box, BoxMode.XYWH_ABS, BoxMode.XYXY_ABS)

    labels = np.asarray([dataset_id_map(predictions[i]["category_id"]) for i in chosen])

    ret.scores = score
    ret.pred_boxes = Boxes(box)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="A script that visualizes the json predictions from COCO or LVIS dataset.")
