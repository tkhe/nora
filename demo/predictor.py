...
from nora.engine.defaults import DefaultPredictor
from nora.utils.video_visualizer import VideoVisualizer
from nora.utils.visualizer import ColorMode
...
class VisualizationDemo:
    def __init__(self, cfg, instance_mode=ColorMode.IMAGE, parallel=False):
        ...
        """
        if isinstance(cfg.dataloader.test.dataset.names, Sequence):
            dataset_name = cfg.dataloader.test.dataset.names[0]
        else:
            dataset_name = cfg.dataloader.test.dataset.names

        self.metadata = MetadataCatalog.get(dataset_name if len(dataset_name) else "__unused")
        ...
        if parallel:
            ...
            self.predcitor = AsyncPredictor(cfg, num_gpus=num_gpu)
        else:
...
class AsyncPredictor:
    ...
    def __init__(self, cfg, num_gpus: int = 1):
        ...
        for gpuid in range(max(num_gpus, 1)):
            cfg.train.device = f"cuda:{gpuid}" if num_gpus > 0 else "cpu"
